...
public abstract class PatternAnalysis {
    ...
    public static boolean analize(LinkedList<Node> conditionals, ArrayList<Node> nodes) {
        ...
        for (Node node : conditionals) {
            ...
            if (node.getBaseStatement() instanceof IfStmt) {
                ...
                if (ifstmt.getCondition() instanceof BinaryExpr) {
                    BinaryExpr conditionExpression = (BinaryExpr) ifstmt.getCondition();
...
//                globalIntervals.add(localIntervals);
        ...
        if (node.getRight() != null) {
            if (node.getRight().getType().equals(Node.NodeType.IF)) {
                ...
                if (!rightNode.hasElse() || isIfElseChain(rightNode)) {
                    if (rightNode.getCondition() instanceof BinaryExpr) {
                        BinaryExpr rightNodeConditionExpression = (BinaryExpr) rightNode.getCondition();
                        if (node.getCondition() instanceof BinaryExpr) {
                            BinaryExpr nodeConditionExpression = (BinaryExpr) node.getCondition();
                            if (nodeConditionExpression.getLeft() instanceof NameExpr) {
                                NameExpr leftSideExpression = (NameExpr) nodeConditionExpression.getLeft();
                                if (rightNodeConditionExpression.getLeft() instanceof NameExpr) {
                                    NameExpr rightNodeleftSideExpression = (NameExpr) rightNodeConditionExpression.getLeft();
                                    if (rightNodeleftSideExpression.getName().equals(leftSideExpression.getName())) {
                                        normalizeTypes(nodeConditionExpression, rightNodeConditionExpression);
                                        if (nodeConditionExpression.getRight().getClass().equals(rightNodeConditionExpression.getRight().getClass())) {
                                            return true;
                                        }
                                    }
                                }
    ...
    private static Interval conditionToInterval(Node condition) {
        if (condition instanceof IfNode) {
            ...
            Expression conditionExpr = ifNode.getCondition();
            if (conditionExpr instanceof BinaryExpr) {
                BinaryExpr binaryExpr = (BinaryExpr) conditionExpr;
...
//                    node.setLeft(toRemoveNode.getRight());
    ...
    */
    private static void normalizeTypes(BinaryExpr firstExpr, BinaryExpr secondExpr){
        if(firstExpr.getRight() instanceof DoubleLiteralExpr
                && secondExpr.getRight() instanceof IntegerLiteralExpr){
            IntegerLiteralExpr secondValueInt = (IntegerLiteralExpr) secondExpr.getRight();
           secondExpr.setRight(new DoubleLiteralExpr(secondValueInt.getBeginLine(), secondValueInt.getBeginColumn(), secondValueInt.getEndLine(), secondValueInt.getEndColumn(), secondValueInt.getValue()));
        }else if(firstExpr.getRight() instanceof IntegerLiteralExpr
                && secondExpr.getRight() instanceof DoubleLiteralExpr){
            IntegerLiteralExpr firstValueInt = (IntegerLiteralExpr) firstExpr.getRight();
           firstExpr.setRight(new DoubleLiteralExpr(firstValueInt.getBeginLine(), firstValueInt.getBeginColumn(), firstValueInt.getEndLine(), firstValueInt.getEndColumn(), firstValueInt.getValue()));
        }
    }
